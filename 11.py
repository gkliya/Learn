...
"""
matches = re.findall('<span class="pre">(.*?)</span>', html)
# print(matches)
column_functions={}
for i,function in enumerate(matches):
    column=i%5  #0,1,2,3,4
    if column in column_functions:
        column_functions[column].append(function)
    else:
        column_functions=[function]
    print(i)

i=1
for functions in column_functions.values():
    for function in functions:
        index=str(i).rjust(2,'0')
        file=open(f'tmp/{index}_{function}.py','w')
        file.close()
        i+=1
...
# for it in matches:
#     i_str= str(i)
#     filename = 'C:\\Users\\test\\PycharmProjects\\lyx0930\\python_te\\' + i_str + '_' + it + '.py'
